...
var averageDollarValue = _.compose(
  _average,
  map(function(c) { return c.dollar_value; })
);
...
var sanitizeNames = map(_.compose(
  _underscore,
  toLowerCase,
  _.prop('name')
));
...
var accountingFormatMoney = _.compose(accounting.formatMoney, _.prop('dollar_value'));

var availablePrices = _.compose(
  join(', '),
  map(accountingFormatMoney),
